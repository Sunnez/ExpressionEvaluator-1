...
#----------------------------------------------------------------
#infix to RPN via shuntingYard (foreign)
LEFT_ASSOC = 0
RIGHT_ASSOC = 1
    ...
    for token in tokens:
        if isOperator(token):
            while len(stackArray) != 0 and isOperator(stackArray[-1]):
                if (isAssociative(token, LEFT_ASSOC) and cmpPrecedence(token, stackArray[-1]) <= 0) or (isAssociative(token, RIGHT_ASSOC) and cmpPrecedence(token, stackArray[-1]) < 0):
...
expression = ConvInfixRpn(expression)
